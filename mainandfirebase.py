...
data_control = ['1','7','30','0','16','30','0','1','8','30','0','18','30','0','0','0','0','0','10000','50000','40','70','512','36','50','1000','50','Danny','password']
...
def read_multi_digit(max_value=9, x=0, y=0, timeout=0):
    num_str = ""
    lcd.move_to(x, y)
    start_time = ticks_ms()
    ...
    while True:
        ...
        # เช็ค timeout
        if timeout > 0 and (ticks_ms() - start_time) > timeout*1000:
            if num_str == "":
                return None  # ไม่มี input ภายใน timeout
            else:
                val = int(num_str)
                if 0 <= val <= max_value:
                    return val
        ...
        if timeout == 0:
            # non-blocking: เช็คครั้งเดียวแล้ว return ถ้าไม่มี key
            return None

        sleep(0.05)

        
def read_number_from_keypad(max_value, x, y):
    lcd.move_to(x, y)
    lcd.putstr("  ")
    lcd.move_to(x, y)
    ...
    while True:
        ...
        if key:
            key = key.strip()
            print("Key:", repr(key))  # Debug ดูว่ากดอะไร
...
def enter_time(time_list, str_label, x, y):
    max_list = [23, 59, 59]
    lcd.clear()
    lcd.move_to(0, y)
    lcd.putstr(str_label)

    for i in range(3):
        lcd.move_to(x + i*3, y + 1)
...
def motor_control(r, l):
    motor_r.duty(r)   # กำหนด duty ขาขวา
    motor_l.duty(l)   # กำหนด duty ขาซ้าย
...
pump_pwm.freq(1000)
pump_pwm.duty(0)   # เริ่มต้น 0 → ปั๊มปิด
...
def select_control_mode(idx, mode):
    global data_control
    lcd.clear()
    lcd.putstr("Select Mode:")
    lcd.move_to(0, 1)
    lcd.putstr(f"1.M 2.Timer 3.T+{mode}")
    ...
    while True:
        ...
        if key == "1":
            ...
            data_control[idx] = '3'
            lcd.clear()
            lcd.putstr(f"Mode: {mode} Control")
            file_operation('w')
            sleep(1)
            break
        elif key == "#":  # ยกเลิก
...
def check_motor(status,left,right):
    s1 = sensor1.value()
    s2 = sensor2.value()
    if s1 == status and s2 == status:
        motor_control(0, 0)  # หยุดมอเตอร์
        return True
    else:
        motor_control(left, right)  # หมุนปิด
        return False
    
def timeout_motor(check_func, timeout=10):
    start = time.time()
    while not check_func():
        sleep(0.1)
        if time.time() - start > timeout:
            print("Warning: motor or sensor error, exit loop")
            motor_control(0, 0)  # หยุดมอเตอร์เผื่อเกิดปัญหา
            break
...
def devControl():
    ...
    # Motor times
    start_time_motor = int(data_control[1])*3600 + int(data_control[2])*60 + int(data_control[3])
    stop_time_motor  = int(data_control[4])*3600 + int(data_control[5])*60 + int(data_control[6])

    # Pump times
    start_time_pump = int(data_control[8])*3600 + int(data_control[9])*60 + int(data_control[10])
    stop_time_pump  = int(data_control[11])*3600 + int(data_control[12])*60 + int(data_control[13])
    

#     # ----------------- Motor Control -----------------
    if data_control[0] == '1':  # Manual
        if data_control[14] == '1' and data_control[15] != '1':  
            # ผู้ใช้สั่งเปิด และมอเตอร์ยังไม่หมุน
            motor_control(0, 512)     # หมุนเปิด
            timeout_motor(check_motor(1,512,0), timeout=15)
            motor_control(0, 0)       # หยุดมอเตอร์
            data_control[15] = '1'    # อัพเดท motorState = หยุด

        elif data_control[14] == '0' and data_control[15] != '0':
            # ผู้ใช้สั่งปิด และมอเตอร์ยังไม่หมุน
            motor_control(512, 0)     # หมุนปิด
            data_control[15] = '1'
            timeout_motor(check_motor(0,0,512), timeout=15)
            motor_control(0, 0)
            data_control[15] = '0'


    elif data_control[0] == '2':  # Auto Mode (Timer)
        active = start_time_motor <= currTime <= stop_time_motor
        
        if active and data_control[15] != '1':  # motorState != 1
            timeout_motor(lambda: check_motor(1,512,0), timeout=15)
            motor_control(0, 0)
            data_control[15] = '1'
            
        elif not active and data_control[15] != '0':
            timeout_motor(lambda: check_motor(0,0,512), timeout=15)
            motor_control(0, 0)
            data_control[15] = '0'

    elif data_control[0] == '3':  # Auto + Light
        active = start_time_motor <= currTime <= stop_time_motor

        if active:
            # หมุนเปิดถ้าแสงต่ำและมอเตอร์ยังไม่หมุน
            if light_value < int(data_control[18]) and data_control[15] != '1':
                timeout_motor(lambda: check_motor(1,512,0), timeout=15)
                motor_control(0, 0)
                data_control[15] = '1'

            # หมุนปิดถ้าแสงสูงและมอเตอร์ยังไม่หมุน
            elif light_value > int(data_control[19]) and data_control[15] != '1':
                timeout_motor(lambda: check_motor(0,0,512), timeout=15)
                motor_control(0, 0)
                data_control[15] = '0'
    ...
    # Manual Mode
    if data_control[7] == '1':
        if data_control[16] == '1' and data_control[17] != '1':  # ผู้ใช้สั่งเปิด & ปั๊มยังไม่ทำงาน
            pump_pwm.duty(int(data_control[22]))
            print("pump start")
            data_control[17] = '1'
        elif data_control[16] == '0' and data_control[17] != '0':  # ผู้ใช้สั่งปิด & ปั๊มยังไม่หยุด
            pump_pwm.duty(0)
            print("pump stop")
            data_control[17] = '0'

    # Timer Mode
    elif data_control[7] == '2':
        active = start_time_pump <= currTime <= stop_time_pump
        if active and data_control[17] != '1':
            pump_pwm.duty(int(data_control[22]))
            data_control[17] = '1'
        elif not active and data_control[17] != '0':
            pump_pwm.duty(0)
            data_control[17] = '0'

    # Humid Mode
    elif data_control[7] == '3':
        active = start_time_pump <= currTime <= stop_time_pump
        if active:
            if humid2 < int(data_control[20]) and data_control[17] != '1':
                pump_pwm.duty(int(data_control[22]))
                data_control[17] = '1'
            elif humid2 > int(data_control[21]) and data_control[17] != '0':
                pump_pwm.duty(0)
                data_control[17] = '0'
                    ...
                    try:
                        ...
                        if mode == "light":
                            if 0 <= val <= 200000:
                                ...
                                file_operation('w')
                                print(f"{'Start' if key=='1' else 'Stop'} {mode} set to {val} lux")
                                return
                            else:
                                lcd.clear()
                        ...
                        if mode == "humid":
                            if 0 <= val <= 100:
                                if key == "1":
                                    data_control[20] = str(val)
                                else:
                                    data_control[21] = str(val)
                                lcd.clear()
                                lcd.putstr(f"Value set: {val}")
                                file_operation('w')
                                print(f"{'Start' if key=='1' else 'Stop'} {mode} set to {val} %")
                                return
                            else:
                                lcd.clear()
                                lcd.putstr("Out of range")
...
def toggle_manual_control(idx, dev):
    global data_control
    lcd.clear()
    data_control[idx] = "1" if data_control[idx] == "0" else "0"
    lcd.putstr(f"{dev}: " + ("ON" if data_control[idx] == "1" else "OFF"))
...
def check_dev_status():
    global data_control
    sn300_status = "ON" if data_control[23] is not None else "OFF"

    sn3000_status = "ON" if data_control[26] is not None else "OFF"

    lux_status = "ON" if data_control[25] is not None else "OFF"
...
def exitfunction():
    key = scan_keypad()  # เช็คครั้งเดียว
    if key:
        while True:
            key = scan_keypad()
            if key in ['#', '*']:
...
def operation_esp32_comm():
    ...
    lcd.move_to(0,1)
    key = read_multi_digit(max_value=21, x=3, y=1, timeout=10)  # timeout=0 → non-blocking
    if key is None:
        return  # ไม่มีการกด → ออกจากฟังก์ชันทันที
    if key == 1: #show date
        ...
        time_list = enter_time(time_list, "time", 5, 0)
        if time_list != -1:
            set_time(time_list)
            lcd.clear()
            lcd.putstr("Time set done!")
            time.sleep(2)
        lcd.clear()
        exitfunction()
    elif key == 5:
        ...
        select_control_mode(0,"light")
        exitfunction()
    elif key == 12:
        select_control_mode(7,"humid")
        exitfunction()
    elif key == 13:
        toggle_manual_control(14,"Motor")
        exitfunction()
    elif key == 14:
        toggle_manual_control(16, "Pump")
        exitfunction()
    elif key == 15:
        check_dev_status()
        exitfunction()
    elif key == 16:
        read_threshold("light")
        exitfunction()
    elif key == 17:#set time start light
        set_device_time_menu("Motor")
    elif key == 18:
        read_threshold("humid")
        exitfunction()
